...
from aris_rl.monitoring.prometheus_query import instant_query, parse_scalar_value
from aris_rl.observation.aggregator import StateAggregator, aggregator_from_config
from aris_rl.observation.prom_feed import apply_instant_metrics, policy_context_from_aris_root

logging.basicConfig(level=os.environ.get("LOG_LEVEL", "INFO"))
logger = logging.getLogger("aris.telemetry_bridge")

KAFKA_EVENTS_TOTAL = Counter(
    "aris_telemetry_bridge_kafka_events_total",
...
def _config_path() -> Path:
    env = os.environ.get("TELEMETRY_BRIDGE_ARIS_CONFIG") or os.environ.get("ARIS_CONFIG")
    if env:
        return Path(env)
    here = Path(__file__).resolve()
    return here.parents[2] / "configs" / "aris.default.yaml"
...
def _kafka_topic() -> str:
    return os.environ.get("KAFKA_TOPIC_ARIS_TELEMETRY", "aris.telemetry.v1")
...
def _poll_prometheus_into(agg: StateAggregator, route: str) -> None:
    ...
    p50 = parse_scalar_value(
        instant_query(base, "aris:http_server_duration:p50_10m", timeout_s=5.0),
    )
    p99 = parse_scalar_value(
        instant_query(base, "aris:http_server_duration:p99_10m", timeout_s=5.0),
    )
    rps = parse_scalar_value(
        instant_query(base, "aris:policy:decisions_per_second", timeout_s=5.0),
    )
    ctx = policy_context_from_aris_root(_load_root())
...
def _apply_kafka_event(agg: StateAggregator, event: dict[str, Any]) -> None:
    ...
    Expected shape (version 1):
      { "schema": "aris.telemetry.v1", "route": "/bench", "latency_ms": 120.0, "ok": true }
    """
    if str(event.get("schema", "")) != "aris.telemetry.v1":
        return
    now = time.time()
...
def _kafka_consumer_loop() -> None:
    ...
    consumer = KafkaConsumer(
        ...
        group_id=os.environ.get("KAFKA_GROUP_ID", "aris-telemetry-bridge"),
...
app = FastAPI(title="ARIS Telemetry Bridge", version="0.1.0")
