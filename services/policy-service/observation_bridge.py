...
logger = logging.getLogger("aris.policy.observation_bridge")
...
def _effective_bridge_block(root: dict[str, Any]) -> dict[str, Any]:
    """Merge YAML with optional env overrides (handy in Kubernetes)."""
    raw = root.get("observation_bridge")
    block: dict[str, Any] = dict(raw) if isinstance(raw, dict) else {}
    if _env_truthy("ARIS_OBSERVATION_BRIDGE_ENABLED"):
        block["enabled"] = True
    url = os.environ.get("ARIS_OBSERVATION_BRIDGE_URL", "").strip()
    if url:
