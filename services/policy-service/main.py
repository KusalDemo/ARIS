...
from aris.policy.v1 import policy_pb2_grpc
from aris_rl.inference.runtime import load_torch_inference_or_none
from aris_rl.safeguards import SafeguardConfig, SafeguardEngine
from config import load_aris_root_config
...
def _start_grpc_server(runtime: _Runtime, port: int) -> grpc.Server:
    ...
    threading.Thread(target=_run, name="aris-policy-grpc", daemon=True).start()
...
async def lifespan(app: FastAPI):
    ...
    root = load_aris_root_config()
    cfg = SafeguardConfig.from_aris_config(root)
...
app = FastAPI(
    title="ARIS Policy Service",
    version="0.2.0",
    lifespan=lifespan,
)


def _request_has_aris_envoy_headers(request: Request) -> bool:
    """True when Envoy (or a client) sent the headers ext_authz forwards for a check."""
    h = request.headers
    return h.get("x-aris-route") is not None or h.get("x-aris-error-rate") is not None
...
def _build_ext_authz_response(request: Request, runtime: _Runtime) -> Response:
    ...
    h = request.headers
    route = h.get("x-aris-route") or request.url.path or "/"
    err_raw = h.get("x-aris-error-rate")
    error_rate = _parse_float_header(err_raw)
    if error_rate is None:
        error_rate = 0.0
    trace_id = h.get("x-aris-trace-id") or h.get("traceparent")

    obs_raw = h.get("x-aris-obs")
    ...
    if obs_raw:
        ...
        if observation is not None and len(observation) != 12:
            return Response(status_code=400, content=b"bad x-aris-obs")

    sr = _parse_float_header(h.get("x-aris-suggested-retry"))
    sb = _parse_float_header(h.get("x-aris-suggested-backoff"))
    st = _parse_float_header(h.get("x-aris-suggested-timeout-ms"))
    ...
    headers = {
        "x-aris-retry": str(out.retry),
        "x-aris-backoff-mult": str(out.backoff_multiplier),
        "x-aris-timeout-ms": str(out.timeout_ms),
        "x-aris-frozen": "1" if out.frozen_active else "0",
    }
    if trace_id:
        headers["x-aris-trace-id"] = trace_id
...
async def envoy_ext_authz_client_path_compat(request: Request, call_next):
    ...
    not /envoy/ext_authz. Treat those as auth checks when ARIS headers are present.
    """
    ...
    if path.startswith("/decide") or path == "/metrics":
        return await call_next(request)
    if not _request_has_aris_envoy_headers(request):
        return await call_next(request)
...
def root() -> dict[str, str]:
    ...
    return {
        ...
        "health": "/health/live",
        "decide": "POST /decide",
        "grpc": "aris.policy.v1.Policy/Decide",
