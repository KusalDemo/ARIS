...
def default_config_path() -> Path:
    """Prefer `ARIS_CONFIG`, else repo `configs/aris.default.yaml` next to this service."""
    env = os.environ.get("ARIS_CONFIG")
    if env:
        return Path(env)
    here = Path(__file__).resolve()
    repo_root = here.parents[2]
    return repo_root / "configs" / "aris.default.yaml"


def load_aris_root_config(path: Path | None = None) -> dict[str, Any]:
