...
from aris_rl.cli import main_benchmark, main_export, main_train
...
def test_main_train_offline_smoke(
    ...
    monkeypatch.setattr(sys, "argv", ["aris-train", "offline", "-c", str(cfg_path), "-n", "cli"])
    main_train()
...
def test_main_train_online_smoke(
    ...
    monkeypatch.setattr(sys, "argv", ["aris-train", "online", "-c", str(cfg_path)])
    main_train()
...
def test_main_export_torch_smoke(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["aris-export", "torch", str(z), str(out_ts), "-a", "dqn"],
    )
    main_export()
...
def test_main_benchmark_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import aris_rl.cli as cli_mod

    monkeypatch.setattr(cli_mod.runpy, "run_module", fake_run)
    monkeypatch.setattr(sys, "argv", ["aris-benchmark", "-c", str(bench), "--arms", "static"])
    main_benchmark()
    assert called.get("yes") is True


def test_main_benchmark_default_config_path(monkeypatch: pytest.MonkeyPatch) -> None:
    if not (REPO_ROOT / "benchmarks" / "run_experiment.py").is_file():
        pytest.skip("benchmarks harness not ported yet (Week 4)")

    import aris_rl.cli as cli_mod

    monkeypatch.setattr(cli_mod.runpy, "run_module", lambda *a, **k: None)
    monkeypatch.setattr(sys, "argv", ["aris-benchmark", "--arms", "static"])
    main_benchmark()
