"""
Load configs/aris.default.yaml (or another YAML file) and check it is complete and sane.

Think of the YAML file as the project's settings menu: training, rewards, safety limits, and
how the policy service runs. We validate early so a typo does not waste hours of training or
crash the live policy server on startup.
"""
...
from aris_rl.observation.features import FEATURE_NAMES

# Every top-level key we expect in the YAML root. If one is missing, load fails fast with a list.
...
# Safeguards = safety rails at serving time (caps, freeze on high errors, rate limits).
...
def load_config(path: str | Path) -> dict[str, Any]:
    """Read YAML from disk and return the root dictionary (must be a mapping, not a list)."""
    p = Path(path)
...
def validate_observation_config(obs: dict[str, Any]) -> None:
    ...
    if tuple(fo) != FEATURE_NAMES:
        msg = "feature_order must match FEATURE_NAMES in aris_rl.observation.features"
        raise ValueError(msg)
...
def validate_reward_config(reward: dict[str, Any]) -> None:
    ...
    Reward weights tell the learner what we care about (success, latency, retries, overload).
    """
...
def validate_action_config(action: dict[str, Any], policy: dict[str, Any]) -> None:
    ...
    for i, x in enumerate(to):
        ...
        if xf < t_min or xf > t_max:
            ...
            raise ValueError(msg)
        # Bins must go low → high so the DQN action grid matches a sensible timeline.
...
def validate_safeguards_config(safeguards: dict[str, Any], policy: dict[str, Any]) -> None:
    ...
    for name, v in (
        ...
        if not (0.0 <= v <= 1.0):
            ...
            raise ValueError(msg)
    # Clear threshold must sit below trip threshold so we un-freeze only after errors drop
    # further — avoids flipping freeze on/off every single request (hysteresis).
    if clr >= thr:
        msg = (
