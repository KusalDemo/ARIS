"""
Command-line tools to train models, export them, check YAML, and score a saved checkpoint.

We keep the flags small on purpose — the real defaults live in the YAML config file.
"""
...
def _find_benchmark_repo_root() -> Path | None:
    """Find the repo folder that contains benchmarks/run_experiment.py (Week 4 harness)."""
...
def _build_train_parser() -> argparse.ArgumentParser:
    ...
    off.add_argument("-c", "--config", type=Path, required=True, help="Path to ARIS YAML config.")
    off.add_argument(
        "-n",
        "--run-name",
        default="offline_run",
    ...
    on.add_argument("-c", "--config", type=Path, required=True, help="Path to ARIS YAML config.")
    on.add_argument(
        "-n",
        "--run-name",
        default="online_run",
...
def _build_validate_parser() -> argparse.ArgumentParser:
    return argparse.ArgumentParser(
        prog="aris-validate-config",
        description="Load an ARIS YAML file and exit 0 if every required section is present.",
    )
...
def _build_eval_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-c", "--config", type=Path, required=True, help="Path to ARIS YAML config.")
    p.add_argument(
        "-m",
        "--model",
        type=Path,
        required=True,
        help="Path to the .zip produced by aris-train.",
    )
    p.add_argument(
        "-n",
        "--episodes",
...
def main_benchmark() -> None:
    """
    Hand off to benchmarks.run_experiment (k6 + Prometheus snapshots).

    Needs a full checkout with benchmarks/; does not start Docker for you.
    """
    ...
    if root is None:
        print(
            "Could not find benchmarks/run_experiment.py — use a full ARIS checkout with "
            "benchmarks/.",
            file=sys.stderr,
        )
        raise SystemExit(2)
...
def main_export() -> None:
    ...
    if args.command == "torch":
        from aris_rl.export.torch_export import export_torchscript
