...
DQN stores each action as one flat index (after the FlattenMultiDiscrete wrapper). PPO stores
three floats (the Box sample). This matches what Stable-Baselines3 puts in the replay buffer.

Offline training checks `replay_buffer.min_transitions` before learning; this class only handles
read/write and rotation on disk.
"""
