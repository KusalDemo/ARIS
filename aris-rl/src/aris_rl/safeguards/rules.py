"""
Safety rails before a suggested retry/backoff/timeout reaches real traffic.

Plain Python only (no FastAPI) so unit tests can hammer the rules quickly.
"""
...
class SafeguardConfig:
    ...
    # (must be < trip threshold so we do not flip-flop on noisy metrics).
    ...
    @classmethod
    def from_aris_config(cls, root: dict[str, Any]) -> SafeguardConfig:
        """Build from the top-level ARIS YAML mapping (same file the RL trainer uses)."""
        sg = root["safeguards"]
        pol = root["policy"]
        return cls(
