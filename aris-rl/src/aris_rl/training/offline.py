...
Offline here means: collect (state, action, reward) tuples into files, then run many gradient
steps without talking to the live cluster. Stable-Baselines3's DQN only supports Discrete actions,
so we flatten MultiDiscrete knobs into one index. PPO is on-policy in SB3, so offline gradient
training is only implemented for DQN; use online training for PPO.
"""
...
from aris_rl.config import effective_training_seed
from aris_rl.envs import ArisMicroserviceEnv
from aris_rl.envs.wrappers import FlattenMultiDiscreteActions
from aris_rl.monitoring import maybe_start_training_metrics
from aris_rl.replay_storage import ReplayStorage, open_storage
from aris_rl.training._device import resolve_training_device
from aris_rl.training.vec_env import make_vec_env
...
def run_offline_training(
    ...
    *,
    ...
    inner = ArisMicroserviceEnv(config, algorithm="dqn")
