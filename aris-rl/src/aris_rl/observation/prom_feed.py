"""
Push Prometheus-style snapshot numbers into the same StateAggregator used in training.

The telemetry bridge polls Prometheus once, then we drop a few synthetic latency samples
so p50/p99 inside the rolling window are meaningful.
"""
...
from aris_rl.observation.aggregator import PolicyContext, StateAggregator
...
def apply_instant_metrics(
    ...
    *,
    ...
    """Record one instant query result at time `now` (seconds)."""
    ...
    if error_rate is not None and math.isfinite(error_rate):
        ...
        # Tiny batch of ok/fail rows so the window's error_rate ≈ er
        n = 20
...
def policy_context_from_aris_root(root: dict[str, Any]) -> PolicyContext:
    """Build PolicyContext from the policy section of configs/aris.default.yaml."""
    pol = root["policy"]
