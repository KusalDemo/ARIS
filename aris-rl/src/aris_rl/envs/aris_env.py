"""
Gymnasium environment for ARIS: pick retry/backoff/timeout, observe a 12-number state, get a reward.

Real microservices are slow and noisy for training loops, so a pluggable backend simulates one step
of latency and load. The observation builder turns those signals into the same vector the live
policy service will see later.
"""
...
from aris_rl.envs.backends import MockBackend, SimulationBackend
...
from aris_rl.rewards.default import compute_reward


class ArisMicroserviceEnv(gym.Env):
    """Small MDP: telemetry aggregation plus a simulator backend."""
    ...
    def __init__(
        ...
        *,
        ...
        # When we create the backend ourselves, reset() can swap in a fresh MockBackend per episode.
    ...
    def reset(
        ...
        *,
        ...
        # Windows and counters start clean each episode.
    ...
    def step(
        ...
        # Observation builder needs the knobs we applied (some features depend on it).
